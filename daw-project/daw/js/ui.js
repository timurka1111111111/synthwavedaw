...
const UI = (() => {
  ...
  function bindModals() {
    ...
    btnLeft?.addEventListener('click', () => {
      const wasOpen = panelLeft?.classList.contains('open');
      closeAllPanels();
      if (!wasOpen) {
    ...
    btnRight?.addEventListener('click', () => {
      const wasOpen = panelRight?.classList.contains('open');
      closeAllPanels();
      if (!wasOpen) {
    ...
    // Show/hide panel buttons based on screen size
    function checkPanelBtns() {
      const mobile = window.innerWidth <= 900;
      if (btnLeft)  btnLeft.style.display  = mobile ? 'flex' : 'none';
      if (btnRight) btnRight.style.display = mobile ? 'flex' : 'none';
      if (!mobile)  closeAllPanels();
    }
    checkPanelBtns();
    window.addEventListener('resize', checkPanelBtns);
  }
